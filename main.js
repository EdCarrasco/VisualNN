...
xhttp.onreadystatechange = function() {
  if (xhttp.readyState === 4 && xhttp.status === 200) {
    const lines = xhttp.response.split("\n");
...
class Layer {
  ...
  createNeuron(options={}) {
    ...
    const padding = 5;
    const neuronHeight = (this.neuronList.length >= 1) ? (this.neuronList[this.neuronList.length-1].position.y + this.radius + padding) : this.height/2;
